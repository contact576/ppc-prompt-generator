...
| `buildMasterPrompt(f)` | Claude Code mode only. Assembles ONE **self-contained** master prompt: 12 sections (mission · operating rules incl. **rule 0 "no pre-installed skills"** · embedded intake · upfront interview · sub-agent roster · **full sub-agent specs** (`secAgentSpecs`, gated on `qcAgentsOn`) · step chain · **embedded creative audit rubric** (`secRubric`, gated on `hasScriptSteps`) · creative 90/100 protocol · Pass 1 gate · Phase B image upload · Pass 2 client doc). The specs + rubric inline everything the sub-agents need so the prompt runs identically on any device with NO installed skills. Concatenates step bodies via `buildReplacements` + strips per-step vault headers. |
