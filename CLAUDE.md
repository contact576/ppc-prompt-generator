...
- **Claude Code** (`claude-code`) — ONE **self-contained** master prompt that wraps the selected step bodies for autonomous end-to-end execution in a Claude Code session, with embedded sub-agent QC + an **embedded creative audit rubric** (formerly the `creative-ad-auditor` skill, now inlined so it needs no install) scoring every creative script ≥90/100 before a human sees it. Built by `buildMasterPrompt(f)` (dispatched from `buildPrompt('master')`). **Portability contract:** the master prompt depends on NO pre-installed skills — every sub-agent spec, rubric, and classification is inlined — so it produces identical output on any employee device (only the MCP connectors are required). Never reintroduce a bare `invoke the X skill` reference without inlining X's full spec.
...
| `buildMasterPrompt(f)` | Claude Code mode only. Assembles ONE **self-contained** master prompt: mission · operating rules incl. **rule 0 "no pre-installed skills"** + **rule 2 vault SAVE-AND-RELOAD** · embedded intake · upfront interview · sub-agent roster · **full sub-agent specs** (`secAgentSpecs`, gated on `qcAgentsOn`) · **STEP 0 MCP pre-flight/resume/spend-ledger** (`secPreflight`) · step chain · **embedded creative audit rubric** (`secRubric`, gated on `hasScriptSteps`) · creative 90/100 protocol · Pass 1 gate · Phase B image upload · Pass 2 client doc. The specs + rubric inline everything the sub-agents need so the prompt runs identically on any device with NO installed skills. **Creative-phase placement (2026-07):** the step chain is SPLIT — `stepBodies` returns `{pid,text}` objects, then `chainThroughGate`/`chainAfterGate` partition at the script-authoring step (`p8`, or `p10` if `p8` absent). The return array is `[…, chainThroughGate, secRubric, sec7, sec8(Pass-1 gate), chainAfterGate, sec9(Phase B), sec10(Pass 2)]` so the 90/100 gate + human approval execute AT Step 8, before P9/P11/P10 consume the scripts — not appended after "END OF CHAIN". Concatenates step bodies via `buildReplacements` + strips per-step vault headers. |
